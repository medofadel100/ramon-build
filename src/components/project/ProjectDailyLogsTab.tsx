    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 font-cairo">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 font-medium">توثيق أحداث الموقع اليومية، الطقس، والصور الميدانية للرجوع إليها لاحقاً.</p>
        </div>
        
        {!isAdding && (
          <button
            onClick={() => setIsAdding(true)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[#c5a880] text-[#0d0e12] text-xs font-bold shadow hover:brightness-110 transition"
          >
      ...
      {isAdding && (
        <div className="bg-[#13151c] border border-[#222634] p-5 rounded-xl space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-right text-xs font-bold text-slate-400 mb-1">تاريخ التقرير</label>
              <input 
                type="date"
                value={newLog.date}
            ...
            <div>
              <label className="block text-right text-xs font-bold text-slate-400 mb-1">حالة الطقس</label>
              <input 
                type="text"
          ...
          <div>
            <label className="block text-right text-xs font-bold text-slate-400 mb-1">تفاصيل التقرير والأعمال المنجزة</label>
            <textarea 
              rows={4}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-500 transition disabled:opacity-50"
            >
      ...
      <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-800 before:to-transparent">
        {logs.map((log) => (
          <div key={log.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
            {/* Timeline dot */}
            <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#0d0e12] bg-[#1a1c24] text-[#c5a880] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-xl z-10">
              <CalendarDays className="w-4 h-4" />
            </div>

            {/* Card */}
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-[#13151c] border border-[#222634] p-4 rounded-xl shadow-lg hover:border-[#c5a880]/30 transition">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <span className="text-xs font-bold text-slate-300">{log.engineerName}</span>
                  </div>
                </div>
                {log.weather && (
                  <div className="flex items-center gap-1 bg-[#1a1c24] px-2 py-1 rounded text-xs text-slate-400">
        ...
        {logs.length === 0 && !isAdding && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-500 font-bold">لا توجد تقارير ميدانية بعد.</p>
          </div>
        )}
      </div>
