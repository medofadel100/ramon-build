    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 font-cairo">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">إرسال واستقبال طلبات استلام الأعمال (IR) والاستفسارات الهندسية (RFI).</p>
        </div>
        
        {canEdit && !isAdding && (
          <button
            onClick={() => setIsAdding(true)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground text-xs font-bold hover:brightness-110 transition"
          >
      ...
      {isAdding && (
        <div className="bg-card border border-border p-5 rounded-xl space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-right text-xs font-bold text-muted-foreground mb-1">نوع الطلب</label>
              <select 
                value={newForm.type}
            ...
            <div>
              <label className="block text-right text-xs font-bold text-muted-foreground mb-1">عنوان الطلب</label>
              <input 
                type="text"
          ...
          <div>
            <label className="block text-right text-xs font-bold text-muted-foreground mb-1">التفاصيل والوصف</label>
            <textarea 
              rows={4}
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-border mt-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded bg-amber-600 text-foreground text-xs font-bold hover:bg-amber-500 transition disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {forms.map((form) => (
          <div key={form.id} className="bg-card border border-border p-5 rounded-xl shadow hover:border-[#c5a880]/30 transition">
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">{new Date(form.dateCreated).toLocaleDateString()}</span>
              </div>
            ...
            <h4 className="text-sm font-bold text-foreground mb-2">{form.title}</h4>
            <p className="text-xs text-muted-foreground leading-relaxed mb-4">{form.description}</p>
            
            <div className="text-[10px] text-muted-foreground flex items-center gap-1 mb-4">
            ...
            {form.status === 'submitted' && user?.role === 'admin' ? (
              <div className="pt-4 border-t border-border space-y-3">
                <p className="text-xs font-bold text-secondary-foreground">رد الاستشاري / الإدارة:</p>
              ...
              <div className={`p-3 rounded-lg border ${form.status === 'approved' ? 'bg-emerald-950/20 border-emerald-900/50' : 'bg-rose-950/20 border-rose-900/50'}`}>
                <p className="text-[10px] text-muted-foreground mb-1">رد بواسطة {form.responder} في {new Date(form.responseDate!).toLocaleDateString()}:</p>
        ...
        {forms.length === 0 && !isAdding && (
          <div className="col-span-1 md:col-span-2 text-center py-12">
            ...
            <p className="text-sm text-muted-foreground font-bold">لا توجد طلبات استلام أو استفسارات حتى الآن.</p>
          </div>
        )}
      </div>
