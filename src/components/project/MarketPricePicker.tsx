...
export function MarketPricePicker({ onSelect, onClose }: MarketPricePickerProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 font-cairo">
      <div className="w-full max-w-2xl bg-[#13151c] border border-[#222634] rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        {/* Search Input */}
        <div className="p-4 border-b border-[#222634]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[#0b0e14] border border-[#222634] rounded-lg pl-3 pr-10 py-2.5 text-sm text-white focus:border-cyan-500/50 focus:outline-none transition"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            ...
            <p className="text-center text-slate-500 text-sm py-8">لا توجد نتائج مطابقة لبحثك.</p>
          ) : (
            filteredMaterials.map(material => (
              <button
                ...
                className="w-full text-right bg-[#0b0e14] border border-[#222634] hover:border-cyan-500/50 hover:bg-[#1a1c24] p-3 rounded-xl transition flex justify-between items-center group"
              >
                <div>
                  <h3 className="text-sm font-bold text-white group-hover:text-cyan-400 transition">{material.name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-slate-400 bg-slate-800 px-2 py-0.5 rounded">{material.category}</span>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-left">
                    ...
                    <span className="text-[9px] text-slate-500">لكل {material.unit}</span>
                  </div>
