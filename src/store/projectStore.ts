...
interface ProjectState {
  ...
  removePurchaseOrder: (poId: string) => Promise<void>;
...
export const useProjectStore = create<ProjectState>((set, get) => {
  ...
  // Purchase Order operations
  // ==========================================
  addPurchaseOrder: async (poInput) => {
    const { currentProject } = get();
    if (!currentProject) return;

    setSaving('addPurchaseOrder');
    try {
      const id = await dbAddPurchaseOrder(currentProject.id, poInput);
      // Re-fetch project to get new PO with its exact PO number generated by the DB
  ...
  updatePurchaseOrder: async (po) => {
    ...
    const updatedPOs = (currentProject.purchaseOrders || []).map(p => p.id === po.id ? po : p);
    set({ currentProject: { ...currentProject, purchaseOrders: updatedPOs } });
    setSaving('updatePurchaseOrder');

    try {
      await dbUpdatePurchaseOrder(currentProject.id, po);
    } catch (e) {
      const err = e as Error;
  ...
  removePurchaseOrder: async (poId) => {
    const { currentProject } = get();
    if (!currentProject) return;

    const updatedPOs = (currentProject.purchaseOrders || []).filter(p => p.id !== poId);
    set({ currentProject: { ...currentProject, purchaseOrders: updatedPOs } });
    setSaving('removePurchaseOrder');

    try {
      await dbDeletePurchaseOrder(currentProject.id, poId);
    } catch (e) {
      const err = e as Error;
