    ...
    <div className="min-h-screen bg-[#0d0e12] flex flex-col font-cairo select-none pb-12">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">إدارة المهندسين والمستخدمين</h1>
            <p className="text-xs text-slate-400 mt-0.5">تحكم في رتب المهندسين وتوزيع الأدوار الفنية والأمنية داخل النظام.</p>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-amber-950/30 bg-amber-950/5 p-4 flex gap-3 text-amber-500 text-xs font-semibold leading-relaxed">
          ...
          <div>
            <p className="text-white font-bold">تنبيه أمني فني</p>
        ...
        {loadingUsers ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#c5a880] border-t-transparent"></div>
            <p className="text-xs text-slate-400">جاري تحميل بيانات المستخدمين...</p>
          </div>
        ) : (
          <div className="overflow-x-auto border border-[#222634] rounded-xl bg-[#13151c] shadow-lg">
                ...
                {users.map((u) => (
                  <tr key={u.uid} className="hover:bg-slate-900/25">
                    <td className="p-3 text-white font-bold flex items-center gap-2">
                      <ShieldCheck className={`h-4 w-4 ${u.role === 'admin' ? 'text-[#c5a880]' : 'text-slate-500'}`} />
                      {u.name}
                    </td>
                    <td className="p-3 text-right dir-ltr font-medium text-slate-400">{u.email}</td>
                    ...
                    <td className="p-3 text-center">
                      <select
                        ...
                        className="w-full bg-[#1a1c24] border border-[#222634] rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-[#c5a880] disabled:opacity-50"
                      >
