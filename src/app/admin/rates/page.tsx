...
import { Save, AlertTriangle, Plus, X, Globe, RefreshCw, Calculator } from 'lucide-react';
              ...
              <div className="flex-1">
                ...
                {c.group === 'rates' && (
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">معدل استهلاك / نسبة هدر</span>
                  </div>
                )}
              </div>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card p-5 rounded-xl border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">المعدلات الهندسية ونسب الهدر</h1>
              <p className="text-xs text-muted-foreground mt-0.5">القاعدة المركزية للمعدلات التي تستخدم في الحصر التلقائي وطلبيات الخامات.</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
        ...
        <div className="rounded-xl border border-border bg-muted/40 p-4 flex gap-3 text-muted-foreground text-xs font-semibold leading-relaxed">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1 font-medium">
              تعديلك للمعدلات هنا سيؤثر على المشاريع الجديدة. هذه المعدلات تُستخدم كمعاملات ثابتة في استخراج طلبيات الخامات من الحصر الفعلي.
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-card rounded-2xl p-6 border border-border">
            {renderGroup('نسب الهدر والمعدلات العامة', constants.filter(c => c.group === 'rates'), 'general')}
            {renderGroup('معدلات المباني', constants.filter(c => c.group === 'rates'), 'masonry')}
            {renderGroup('معدلات المحارة والدهانات', constants.filter(c => c.group === 'rates'), 'plastering')}
            {renderGroup('معدلات السيراميك والرخام', constants.filter(c => c.group === 'rates'), 'flooring')}
            {renderGroup('معدلات السباكة', constants.filter(c => c.group === 'rates'), 'plumbing')}
            {renderGroup('معدلات الكهرباء', constants.filter(c => c.group === 'rates'), 'electrical')}
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-border bg-[#1a1c24]">
              <h3 className="text-lg font-bold text-foreground">إضافة معدل جديد أو نسبة هدر</h3>
              <button 
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-muted-foreground mb-2">اسم المعدل أو النسبة</label>
                <input
                  type="text"
                  placeholder="مثال: نسبة هدر الجبس بورد"
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground mb-2">القيمة الافتراضية</label>
                <input
                  type="number"
