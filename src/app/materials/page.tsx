      ...
      <div className="flex-1 p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">تابع أسعار مواد التشطيب والتأسيس من مختلف المتاجر أولاً بأول.</p>
          </div>
          <div className="flex flex-wrap gap-2">
             <button
               onClick={handleExportToExcel}
               className="bg-muted/50 border border-border hover:bg-[#c5a880]/10 hover:border-[#c5a880]/50 hover:text-[#c5a880] transition-all rounded-lg p-2 flex items-center gap-2 cursor-pointer"
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="w-full lg:w-64 space-y-6 shrink-0 lg:sticky lg:top-6 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
             <div className="bg-card/80 border border-border rounded-xl p-5 backdrop-blur-xl">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="space-y-4">
              <div className="relative group">
                <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-cyan-400 transition-colors" />
                <input
                  type="text"
                  placeholder="ابحث عن خامة، أداة، أو علامة تجارية..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full bg-card/80 border border-border rounded-xl py-4 pr-12 pl-4 text-foreground placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all shadow-lg"
                />
              </div>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                      selectedPhase === phase
                        ...
                        : 'bg-card/80 text-muted-foreground border border-border hover:bg-slate-800'
                    }`}
                  >
                    {phase}
                  </button>
              ...
              {availableBrands.length > 1 && (
                <div className="flex gap-2 flex-wrap pt-2 border-t border-border/50">
                  <span className="text-sm text-muted-foreground py-1.5 px-2">الماركات:</span>
                  {availableBrands.map(brand => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                        selectedBrand === brand
                          ? 'bg-indigo-500 text-foreground shadow-[0_0_10px_rgba(99,102,241,0.4)]'
                          : 'bg-card text-muted-foreground border border-border hover:bg-slate-800'
                      }`}
                    >
                      {brand}
                    </button>
            ...
            {loading ? (
              ...
              <div className="text-center py-20 bg-card/50 rounded-xl border border-dashed border-border">
                <p className="text-muted-foreground">لا توجد خامات مطابقة للبحث.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {filteredMaterials.map(material => (
                  <Link href={`/materials/${material.id}`} key={material.id}>
                    <Card className="relative bg-card/80 border-border hover:border-cyan-500/50 hover:bg-[#13151c] transition-all duration-500 overflow-hidden group cursor-pointer h-full flex flex-col hover:shadow-[0_0_30px_rgba(6,182,212,0.15)] hover:-translate-y-1">
                      ...
                      <div className="p-6 flex-1 flex flex-col relative z-10">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex flex-wrap gap-2">
                            <span className="text-xs font-medium px-3 py-1 rounded-full bg-accent/80 text-secondary-foreground border border-border/50 backdrop-blur-sm">
                        ...
                        <h3 className="text-xl font-bold text-foreground group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-emerald-400 transition-all duration-300 mb-4 leading-relaxed line-clamp-2">
                          {material.name}
                        </h3>
                        
                        <div className="space-y-4 mt-auto">
                           <div className="flex justify-between items-end border-t border-border/60 pt-4">
                              <p className="text-sm font-medium text-muted-foreground">أقل سعر بالسوق</p>
                              <div className="text-right flex items-baseline gap-1.5">
                                ...
                                <span className="text-sm font-medium text-muted-foreground">ج.م / {material.unit}</span>
                              </div>
                           </div>
                              ...
                              {material.sources.slice(0, 3).map((source, idx) => (
                                 <button 
                                   ...
                                   className="flex items-center gap-1.5 text-[11px] font-medium text-secondary-foreground bg-muted/80 hover:bg-slate-800 px-2.5 py-1.5 rounded-md border border-border shadow-sm hover:border-cyan-500/50 hover:text-cyan-400 transition-all cursor-pointer"
                                 >
                              ...
                              {material.sources.length > 3 && (
                                <span className="text-xs font-medium text-muted-foreground bg-muted/50 px-2 py-1 rounded-md">+{material.sources.length - 3} متاجر</span>
                              )}
                           </div>
                        </div>
                      </div>
